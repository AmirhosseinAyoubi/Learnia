...
it to the auth-service ``/api/v1/auth/validate`` endpoint.  The ai-service
...
async def require_api_key(x_api_key: str = Header(..., alias="X-API-Key")) -> str:
    ...
    Returns:
        The validated API key string (passed through to the route handler
        so it can be forwarded to downstream service calls).
